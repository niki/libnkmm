...
//! 2005.05.20 open() で CreateFile() を呼んでいるようなので元に戻す
//! 2005.04.26 indows のときはファイル処理に API を使うようにした
//====================================================================
#ifdef __debugprint
#include <stdio.h>  //f*()
...
*/
rei::string &StringList::basic_Add(const rei::string &rp) {
  rei::string &item = Strings.push_back(rp);
...
/*!
	...
	@return 追加した String クラスの参照
*/
rei::string &StringList::basic_Add(const rei::string &rp, int len) {
  rei::string &item = Strings.push_back();
  if (len > 0) {
    item.assign(rp, len);
  }
#if USE_STRINGLIST_RETURNCODE
...
/*!
	...
	@return 追加した String クラスの参照
*/
rei::string &StringList::basic_Insert(int index, const rei::string &rp) {
  rei::string &item = Strings.insert(index, rp);
#if USE_STRINGLIST_RETURNCODE
...
/*!
	...
	@return 追加した String クラスの参照
*/
rei::string &StringList::basic_Insert(int index, const rei::string &rp, int len) {
  rei::string &item = Strings.insert(index);
  if (len > 0) {
    item.assign(rp, len);
  }
#if USE_STRINGLIST_RETURNCODE
...
/*!
	...
	@return 追加した String クラスの参照
*/
rei::string &StringList::basic_Assign(int index, const rei::string &rp) {
  Strings[index] = rp;
...
/*!
	...
	@return 追加した String クラスの参照
*/
rei::string &StringList::basic_Assign(int index, const rei::string &rp, int len) {
  if (len == 0) {
    Strings[index] = "";
  } else {
...
/*!
	...
	@param[in] rp 文字列
	@return 追加した rei::string クラスの参照
*/
rei::string &StringList::basic_Concatenate(int index, const rei::string &rp) {
  Strings[index] += rp;
  return Strings[index];
}
...
/*!
	...
	@return 追加した rei::string クラスの参照
*/
rei::string &StringList::basic_Concatenate(int index, const rei::string &rp, int len) {
  if (len == 0) {
    //
  } else {
...
//rei::string &StringList::add(const rei::string &fmt, ...)
rei::string &StringList::add(const char *fmt, ...) {
  if (*fmt) {
    rei::string s;
...
//rei::string &StringList::assign(int index, const rei::string &fmt, ...)
rei::string &StringList::assign(int index, const char *fmt, ...) {
  if (*fmt) {
    rei::string s;
    s.extendBuffer(0x1000);
...
//rei::string &StringList::concatenate(int index, const rei::string &fmt, ...)
rei::string &StringList::concatenate(int index, const char *fmt, ...) {
  if (*fmt) {
    rei::string s;
    s.extendBuffer(0x1000);
...
//rei::string &StringList::insert(int index, const rei::string &fmt, ...)
rei::string &StringList::insert(int index, const char *fmt, ...) {
  if (*fmt) {
    rei::string s;
    s.extendBuffer(0x1000);
...
static int fnSortStringUp(const rei::string &a, const rei::string &b) {
...
static int fnSortStringDown(const rei::string &a, const rei::string &b) {
...
static int fnSortLengthUp(const rei::string &a, const rei::string &b) {
...
static int fnSortLengthDown(const rei::string &a, const rei::string &b) {
...
/*!
	...
	@param[in] fnCmp 比較関数
*/
void StringList::sort(int (*fnCmp)(const reirei::string &, const rei::string &)) {
...
void StringList::sort(int sortUp) {
  static int (*s_fnCmp[2])(const rei::string &, const rei::string &) = {
...
void StringList::deleteBlankLines() {
  for (int i = count() - 1; i >= 0; --i) {
    rei::string &s = Strings[i];

    //文字列をスキャンして、指定された文字群以外が最初に出現する位置を見つけます
    const int pos = rei::strspn(s.c_str(), " \t");
...
/*!
	行コメント処理
*/
void StringList::removeLineComment(const rei::string &lineCommentWord) {
  for (int i = count() - 1; i >= 0; i--) {
    rei::string &s = Strings[i];
...
*/
int StringList::loadFromFile(const rei::string &fileName, bool concat) {
  File file;
...
/*!
	...
	@return size
*/
int StringList::toString(rei::string &out, const char delimiter) {
  if (Strings.empty()) {
    out = rei::string("");
    return 0;
  }
...
/*!
	...
	@return 0:成功, 1:失敗
*/
int StringList::saveToFile(const rei::string &fileName) {
  File file;
