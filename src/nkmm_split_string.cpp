...
SplitString::SplitString(const nkmm::string &text, const char *delimiters, int flag)
    : textOrg_(text), text_("") {
  init();
...
/*!
	...
	@return	トークン
*/
nkmm::string SplitString::at(int index) const {
#if 1
...
*/
void SplitString::split(const nkmm::string &text, const nkmm::string &delimiters, int flag) {
  if (!text.empty()) {
    textOrg_ = text;
  }
