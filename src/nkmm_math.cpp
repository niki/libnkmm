...
static int align(char* buf, int toLen, int option) {
  int fromlen = nkmm::strlen(buf);  // 長さ
  ...
  if (toLen == 0) {  // 長さの指定は自動なので fromlen に合わせる
    ...
    // 頭のスペースを０に置き換える
    if (option & nkmm::math::kFillZero) {
      ch = (char)'0';
    } else {
      ch = (char)' ';
    }

    if (option & nkmm::math::kAlignLeft) {  // 左寄せ
      // 文字移動
    ...
    } else if (option & nkmm::math::kAlignRight) {  // 右寄せ
      // うめうめ
...
char* DecToHex(int dec, char* buf, int len, const char* hexPrefix, int option) {
  ...
  if (option & kPrefix) {
    int i;
    char temp[256];
    nkmm::strcpy(temp, buf);

    for (i = 0; hexPrefix[i]; ++i) {
      buf[i] = hexPrefix[i];
    }

    nkmm::strcpy(buf + i, temp);
  }
...
int StrToInt(const char* buf, const char* hexPrefix) {
  ...
  {
    int len = nkmm::strlen(hexPrefix);

    if (len > 0 && nkmm::strncmp(p, hexPrefix, len) == 0) {
      radix = 16;  // 16 進数
      p += len;
    }
  }
