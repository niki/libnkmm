...
static void fn_lisp_random(Lisp &lisp, LispArgs &args) {
  ...
  switch (args[1].type()) {
  case nkmm::Var::kTypeInt:
    ret = static_cast<LispInt>(rand() % args[1].asInt());
    break;

  case nkmm::Var::kTypeFloat: {
...
void Lisp::push() {
  ...
  ::OutputDebugStringW(L"*** LISP PUSH: vars size " +
                       nkmm::wstring(static_cast<int>(vars_.size()), L"%d") + L", \n");
}
          ...
          if (vmap.isOutputDebugString_ && args[0].str()[0] != '@') {
            ...
            for (int i = 1, n = args.size(); i < n; i++) {
              ...
              case Var::kTypeUnknown:
                ...
                break;
              case Var::kTypeFloat:
                s += " [f]" + nkmm::string(args[i].ff(), "%f");
                break;
              case Var::kTypeInt:
                s += " [d]" + nkmm::string(args[i].i(), "%d");
                break;
              case Var::kTypeBool:
                s += " [b]" + nkmm::string(args[i].asBool() ? "true" : "false");
                break;
              case Var::kTypeString:
                s += " [s] \"" + args[i].str() + "\"";
                break;
              case Var::kTypePointer:
                s += " [p]" + nkmm::string(reinterpret_cast<Int>(args[i].ptr()), "%08x");
                break;
              }
            }
