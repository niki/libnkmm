...
IniFile::IniFile(const rei::string &fileName) {
  open(fileName);
}
...
*/
void IniFile::open(const rei::string &fileName) {
  // リスト作成
...
*/
bool IniFile::isSection(rei::string &s) {
...
/*!
	...
	@return データ
*/
rei::string IniFile::getKeyData(const rei::string &section, const rei::string &key) {
  StringList *list = pList_;
  int state = 0;

  for (int i = 0; i < list->count(); i++) {
    rei::string &item = list->Strings[i];
    ...
    case FIND_SECTION_THIS_NOSECTION: {
      ...
      if (index >= 0) {
        rei::string t_key = item.subString(0, index).trim(" \t");
    ...
    case NOTFIND_SECTION_THIS_SECTION: {
      ...
      if (index1 >= 0 && index2 >= 0) {
        rei::string t_section = item.subString(index1 + 1, index2 - index1 - 1).trim(" \t");
...
/*!
	...
	@param[in] data データ
*/
void IniFile::setKeyData(const rei::string &section, const rei::string &key,
                         const rei::string &data) {
  StringList *list = pList_;
  int state = 0;
...
/*!
	...
	@return キーリスト
*/
StringList *IniFile::enumKey(StringList *key, const rei::string &section) {
  StringList *list = pList_;
  int state = 0;
...
/*!
	...
	@param[in] section セクション名
*/
void IniFile::deleteSection(const rei::string &section) {
  StringList *list = pList_;
  int state = 0;
...
/*!
	...
	@param[in] key キー名
*/
void IniFile::deleteKey(const rei::string &section, const rei::string &key) {
  StringList *list = pList_;
  int state = 0;
...
/*!
	...
	@return キーの値
*/
rei::string IniFile::readString(const rei::string &section, const rei::string &key,
                                const rei::string &def) {
  rei::string data = getKeyData(section, key);
...
/*!
	...
	@return 数値
*/
int IniFile::readInteger(const rei::string &section, const rei::string &key, int def) {
  rei::string data = getKeyData(section, key);
...
/*!
	...
	@return 結果
*/
bool IniFile::readBool(const rei::string &section, const rei::string &key, bool def) {
  rei::string data = getKeyData(section, key);
...
*/
void IniFile::writeString(const rei::string &section, const rei::string &key,
                          const rei::string &write) {
...
/*!
	...
	@param[in] write 書き込むデータ
*/
void IniFile::writeInteger(const rei::string &section, const rei::string &key, int write) {
  rei::string data(write, "%d");
...
*/
void IniFile::writeBool(const rei::string &section, const rei::string &key, bool write,
                        bool string) {
  if (string) {
    if (write) {
    ...
    rei::string data(static_cast<int>(write), "%d");
    setKeyData(section, key, data);
  }
}
