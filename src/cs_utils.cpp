...
#include "mx_memory.h"
#include "mx_basis.h"
#include "mx_strlib.h"  // 文字列処理
#include "mx_math.h"    // 数値処理
...
void CommentToSpace(char* wp, const char* lines, const char* blockBegin, const char* blockEnd,
  ...
  // 行コメントの長さ
  if (lines) {
    comm.line.len = mx::strlen(lines);  // 行コメント
  }

  // ブロックコメントの長さ
  if (blockBegin && blockEnd) {
    comm.blockBegin.len = mx::strlen(blockBegin);  // ブロックコメント開始
    comm.blockEnd.len = mx::strlen(blockEnd);      // ブロックコメント終了
  }
