...
// 数値を文字列に変換
#ifdef NCRT
#define MCR_atoi(x) nkmm::math::StrToInt(x)
#define MCR_itoa(x, dest, radix) nkmm::math::IntToStr(x, dest, (radix) == 10 ? NULL : "0x")
#define MCR_atof(x) nkmm::math::StrToFloat(x)
#else
#include <stdlib.h>
#include <math.h>
...
#define MCR_ftoa(x, dest) nkmm::math::FloatToStr(x, dest, 0)
