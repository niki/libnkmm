...
SplitString::SplitString(const rei::string &text, const char *delimiters, int flag)
    : textOrg_(text), text_("") {
  init();
...
/*!
	...
	@return	トークン
*/
rei::string SplitString::at(int index) const {
#if 1
...
*/
void SplitString::split(const rei::string &text, const rei::string &delimiters, int flag) {
  if (!text.empty()) {
    textOrg_ = text;
  }
