...
#define NEXT_TOKEN(p) (p + rei::strlen(p) + 1)

static int local_SearchFolders(rei::string &path, const rei::string &patIn,
                               const rei::string &patEx, const rei::string &dirIn,
                               const rei::string &dirEx);
static int fnDefaultCallback(const rei::string &fname, void *data);

static int (*s_fnCallback)(const rei::string &, void *) = fnDefaultCallback;
...
* @return 0 とりあえず０を返す
*/
static int fnEnumFoldersCallback(const rei::string &fname, void *data) {
  ...
  // リストにフォルダを追加
  rei::string &item = ef->files->basic_Add(fname);
...
* @return 0  とりあえず０を返す
*/
static int fnEnumFilesCallback(const rei::string &fname, void *data) {
...
static int local_EnumFilesToList(const u_long reqattr, const u_long unreqattr) {
  ...
  rei::strcpy(t_path, ef->dir);
  rei::strcat(t_path, "\\");
  rei::strcat(t_path, ef->pat);
  ...
  do {
    // "." ".." は無視
    if (rei::strcmp(ef->fd.cFileName, ".") && rei::strcmp(ef->fd.cFileName, "..")) {
...
*/
void SetCallback(int (*fnCallback)(const rei::string &, void *), void *data) {
  if (fnCallback) {
...
* @return 0:なし, 1:あり
*/
int SubFolderExists(const rei::string &dir) {
  int sub = 0;
  HANDLE hFind;        // 検索用のハンドル
  WIN32_FIND_DATA fd;  // 列挙情報取得用
  char t_path[256];

  rei::strcpy(t_path, dir.c_str());
  rei::strcat(t_path, "\\*.*");
  ...
  do {
    // "." ".." は無視
    if (rei::strcmp(fd.cFileName, ".") && reireirei::strcmp(fd.cFileName, "..")) {
...
* @endcode
*/
StringList *EnumFolders(const rei::string &dir) {
...
* @endcode
*/
StringList *EnumFiles(const rei::string &dir, const rei::string &mask) {
  int tok_cnt;
  char t_mask[256];
  char *maskP = t_mask;
  ...
  // トークンの抜き出し
  tok_cnt = rei::sjis::ExtractTokenWithoutEmpty(t_mask, mask.c_str(), " ,;",
                                                "");  // トークンは ' ' ',' ';'
...
*/
static int local_SearchFiles(rei::string &path, const rei::string &patIn, const rei::string &patEx,
                             const rei::string &dirIn, const rei::string &dirEx) {
  int nCount = 0;
  ...
  char t_mask[256];
  char *maskP = t_mask;
  int tok_cnt = rei::sjis::ExtractTokenWithoutEmpty(t_mask, patIn.c_str(), " ,;", "");

  for (int i = 0; i < tok_cnt; i++) {
    // パターン名作成
    rei::string searchPath = path + "\\" + maskP;
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
      ...
      do {
        // . と .. は無視する
        if (rei::strcmp(fd.cFileName, ".") != 0 && rei::strcmp(fd.cFileName, "..") != 0) {
          // フォルダ属性か調べる
          if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            // 除外パターンのチェック
            if (patEx.length() > 0) {
              if (rei::sjis::CheckMasks(fd.cFileName, patEx.c_str())) {
                nCheck = 0;
              }
            }
            ...
            if (nCheck == 1) {
              // ファイル名を加える
              rei::string s = path + "\\" + fd.cFileName;
...
static int local_SearchFolders(rei::string &path, const rei::string &patIn,
  ...
  char t_mask[256];
  char *maskP = t_mask;
  int tok_cnt = rei::sjis::ExtractTokenWithoutEmpty(t_mask, dirIn.c_str(), " ,;", "");
      ...
      do {
        ...
        if (rei::strcmp(fd.cFileName, ".") != 0 && rei::strcmp(fd.cFileName, "..") != 0) {
          ...
          if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            // 除外パターンのチェック
            if (dirEx.length() > 0) {
              if (rei::sjis::CheckMasks(fd.cFileName, dirEx.c_str())) {
                nCheck = 0;
              }
            }
            ...
            if (nCheck == 1) {
              // ファイル名作成
              rei::string folderPath = path + "\\" + fd.cFileName;
...
* @return ヒットした数
*/
int SearchFiles(const rei::string &path, const rei::string &patIn, const rei::string &patEx,
                const rei::string &dirIn, const rei::string &dirEx) {
  rei::string path_ = path;
...
StringList *EnumWindow(bool visibleOnly) {
  ...
  for (0; hWnd; hWnd = ::GetNextWindow(hWnd, GW_HWNDNEXT)) {
    ...
    {
      //rei::string &s = list->Add( "%s@%s,%.8x", szCaption, szClassName, ::GetWindowLong( hWnd, GWL_STYLE ));
      rei::string &s = list->add("%s@%s", szCaption, szClassName);
      s.tag(0, (u_long)hWnd);
      s.tag(1, nStyle);
    }
...
HWND GetWindowHandleFromProcess(const DWORD targetID, const char *caption) {
  ...
  do {
    ...
    if (targetID == processID) {
      ...
      if (rei::sjis::SearchString(szCaption, caption)) {
        return hWnd;
      }
    }
...
*/
void DoFind(StringList *fileList, const rei::string &dir, const rei::string &mask,
            const rei::string &excludeDirMask, const rei::string &excludeFileMask) {
  HANDLE hFind;
  WIN32_FIND_DATA fd;
  ...
  do {
    ...
    if (rei::strcmp(fd.cFileName, ".") != 0 && rei::strcmp(fd.cFileName, "..") != 0) {
      ...
      if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
        ...
        // 再起検索
        if (rei::sjis::CheckMasks(fd.cFileName, excludeDirMask.c_str())) {
        } else {
          DoFind(fileList, dir_.c_str(), mask, excludeDirMask, excludeFileMask);
        }

      } else {  // ファイル
        if (rei::sjis::CheckMasks(fd.cFileName, excludeFileMask.c_str())) {
        } else {
          if (rei::sjis::CheckFileMasks(fd.cFileName, mask.c_str())) {
            dir_ = dir;
            ...
            // ファイル名追加
            rei::string &s = fileList->add(dir_.c_str());
...
*/
void DoFind2(StringList *fileList, const rei::string &dir, const rei::string &prefix,
             const rei::string &ext, int subLevel) {
  HANDLE hFind;
  WIN32_FIND_DATA fd;

  rei::string t_dir = dir;
  ...
  do {
    ...
    if (rei::strcmp(fd.cFileName, ".") != 0 && rei::strcmp(fd.cFileName, "..") != 0) {
      ...
      if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
        ...
        // 指定外
        if (prefix.c_str()[0] == '!') {
          rei::string prefix1 = fd.cFileName;
          rei::string prefix2 = prefix.c_str() + 1;
          rei::string ext1 = rei::string(fd.cFileName).asFileExt();
          rei::string ext2 = ext;

          if (!rei::sjis::SearchString(prefix1.upCase().c_str(), prefix2.upCase().c_str()) &&
...
void SpecialPathToPath(rei::string &str) {
...
void PathToSpecialPath(rei::string &str) {
  for (int i = 0; i < s_folderMax; i++) {
    char path[MAX_PATH];
...
char *GetFolder(HWND Handle, char *dirPath, const char *title, const char *defaultPath) {
  ...
  rei::Memory::Zero(&BI, sizeof(BROWSEINFO));
  //	rei::Memory::Zero(dirPath, MAX_PATH);
  rei::Memory::Zero(FName, MAX_PATH);
...
bool CreateShortcut(const char *name, const char *dir, const char *fileName, const char *param) {
  rei::string sTarget;    //起動させたいアプリケーションのファイル名
  rei::string sCmdParam;  //アプリケーションに渡すパラメータ
  rei::string sSCFName;   //ショートカットそのもののファイル名
...
*/
rei::string GetExecutable(const rei::string &strExtension) {
  DWORD dwOut;
  HRESULT hr;
  TCHAR fname[MAX_PATH + 10];
  rei::string ext;

  rei::string strCommand = "";
